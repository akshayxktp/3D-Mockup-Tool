...
export default function ThreeStage3D({ effectId: forcedEffectId }: { effectId?: string } = {}) {
  ...
  useEffect(() => {
    ...
    const dispose = def.init(stage, canvas, {
      ...
      onPickFocus: (x, y) => {
        ...
        s.setParam(effectId, 'blurFocusX', Math.round(x * 100) / 100);
        s.setParam(effectId, 'blurFocusY', Math.round(y * 100) / 100);
      },
    ...
    const threeRenderer: IRenderer = {
      ...
      setTransparentBackground: (on) => { threeEngineRef.current?.setTransparentBackground?.(on); },
  ...
  // 'Off' and any unknown value collapse to 0, which renders no overlay at all.
  const gridDivisions = has('grid') ? ({ '3×3': 3, '6×6': 6 } as Record<string, number>)[String(p.grid ?? 'Off')] ?? 0 : 0;

  // Applied to the <canvas> alone, not the wrapper — so the Adjustments group
  // grades the rendered device and leaves the stage backdrop untouched. Built by
  // the shared helper the image export replays, so preview and export grade the
  // device identically.
  const filters = gradeFilter(p, has);

  const canvasStyle: React.CSSProperties = {
    filter: filters || undefined,
    // Armed for a focus pick — say so with the pointer, since the click means
    // something different from its usual part-select.
    cursor: isOn(p.blurFocusPick) ? 'crosshair' : undefined,
  ...
  return (
    ...
    <div className="stage-wrap" style={{ containerType: 'size' } as React.CSSProperties}>
      ...
      >
        ...
        {/* Composition grid. Sits inside the stage box, which is already sized
            to the scene's aspect, so the divisions land on the frame the
            export will produce rather than on the surrounding letterbox. */}
        {gridDivisions > 1 && (
          <div className="three3d-lens three3d-grid" aria-hidden="true">
            {Array.from({ length: gridDivisions - 1 }, (_, i) => (
              <span key={`v${i}`} className="three3d-grid-line three3d-grid-v" style={{ left: `${((i + 1) / gridDivisions) * 100}%` }} />
            ))}
            {Array.from({ length: gridDivisions - 1 }, (_, i) => (
              <span key={`h${i}`} className="three3d-grid-line three3d-grid-h" style={{ top: `${((i + 1) / gridDivisions) * 100}%` }} />
            ))}
          </div>
        )}
